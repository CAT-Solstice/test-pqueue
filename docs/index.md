...
While trying to optimize a custom bounded priority queue, I ran into a pathological case.
You can find the repo [here](https://github.com/CAT-Solstice/test-pqueue).
...
use `opt-level` this way with `criterion`, it is an easy way to make the point.
...
The example and the bench are building a top-k of distinct pair-like `(id, dist)` elements from a collection of random elements. The
constraint of unicity over `id` make the use of a binary heap unefficient, so instead I maintain a sorted `Vec` inserting new element
...
The queue is using a [binary search](https://doc.rust-lang.org/std/primitive.slice.html#method.binary_search_by) on the vec to
find the position to insert new element. They should be ordered by `dist` then by `id` and [as you may know](https://floating-point-gui.de/errors/comparison/),
...
At this point you may think: "So many cmoves! Your compare function is odd!". While one can argue about this implementation,
I used it here because the optimization level change the emitted assembly with this particular one. I tested other implementations,
and they consistently produce either jumps (fast) or moves (slow).
...
of the `f32`, which should add some bit twiddling operations. Using `opt-level=3`, here is the emitted assembly:
...
compare function which uses `total_cmp`, the performance plummets event more and is now 3x slower.
...
The performance regression tells us there are benchmarks for which conditional moves are faster than conditional jumps, and I bet
they were conducted by people who know better than I do. Benchmarking is hard, and I might be stretching myself a bit too much here.
I hope you enjoyed the ride and learned something in the meantime!
